...
if ($_SERVER['REQUEST_METHOD']== 'POST') {
    $datelocal = date('Y-m-d h:i:s', time());  
  ...
  exit;
    ...
    $insertars = $con -> prepare("INSERT INTO `sentimientos`(`sentimiento`,`fecha`, `id_usuario`) 
    VALUES ('$sentimiento','$datelocal','$varid')");
    $insertars ->execute();
    $_SESSION['mensaje']='<div class="alert alert-info d-flex align-items-center mt-3" role="alert">
    <svg class="bi flex-shrink-0 me-2" width="24" height="24" role="img" aria-label="Info:"><use xlink:href="#exclamation-triangle-fill"/></svg>
    <div>
     Test de sentimiento enviado correctamente
    </div>
  </div>';
  header("location:../../php/test.php");
   }
