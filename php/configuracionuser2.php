...
$query = $con -> prepare ("SELECT * FROM usuario WHERE id_usuario = '$id'");
$query ->execute();
$resultado = $query ->fetchAll(PDO::FETCH_ASSOC);

//verificar que solo usuario sin datos persona entre 

$persona = $con -> prepare ("SELECT count(id_persona) FROM persona WHERE id_usuario = '$id'");
$persona ->execute();
if ($persona->fetchColumn()>0) {
   header("location:configuracionuser.php");
   }
?>
<!DOCTYPE html>
<html lang="en">
...
<nav class="menu">
   ...
   <div class="usuario">
    ...
    <div class="nav flex-column nav-pills me-3 mt-5 p-2" id="v-pills-tab" role="tablist" aria-orientation="vertical">
      ...
      <button class="nav-link" id="v-pills-datos-tab" data-bs-toggle="pill" data-bs-target="#v-pills-datos" type="button" role="tab" aria-controls="v-pills-datos" aria-selected="false" disabled>Complemento de datos</button>
...
<!------------------- Primer apartado ------------------->
            ...
            <form action="../login/php/modipersona_be.php" method="POST">
              ...
              <p><input class="form-control" type="email" value="<?php echo $row['correo']?>" name="correo" required></p>
                  <?php
              ...
              <button class="btn btn-dark col" value="modificarusuario" name="accion">Modificar</button>
